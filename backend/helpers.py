...
async def insert_into_message_origin_tracking(
    message_id, my_message_id, channel_id, conn
):
...
async def find_empty_days(conn):
    ...
    row: asyncpg.Record
    for row in rows:
        new_rows.append({"message": row["message_id"], "author": row["user_id"]})
    return new_rows
    ...
    async def convert(self, ctx: commands.Context, argument: str):
        ...
        if int(argument) in month_range:
            ...
            raise commands.BadArgument(
                "{} is not a valid day in the current month of {}".format(
                    argument, now.strftime("%B")
                )
            )
...
class Helper(commands.Cog):
    ...
    async def force_add_message(
        self, ctx: commands.Context, channel: discord.TextChannel, message: int
    ):
        ...
        try:
            await backend.discord_events.on_reaction_add.new_inktober(
                fetched_message, self.bot
            )
    ...
    async def force_alter_day(
        ...
        log.info(
            "{} {} {} {} {} {}".format(
                channel, message, day, type(channel), type(message), type(day)
            )
        )
        try:
            fetched_message: discord.Message = await channel.fetch_message(message)
    ...
    async def find_original(
        ...
        try:
            my_message_id, my_channel_id = await fetch_from_tracking_table(
                message, self.bot.db
            )
        except TypeError as TE:
            ...
            await ctx.send(
                "https://discordapp.com/channels/{}/{}/{}".format(
                    ctx.guild.id, my_channel_id, my_message_id
                )
            )
    ...
    @commands.command(
        pass_context=True, brief="Given a user find the amount of posts + what day"
    )
    @commands.check(backend.command_checks.is_authed)
    async def inktober_info(self, ctx: commands.Context, user: discord.Member):
        ...
        embed.add_field(
            name="Days", value=", ".join(str(day) for day in days.split("|"))
        )
        embed.set_author(name=user.name, icon_url=user.avatar_url)

        await ctx.send(embed=embed)

    @commands.command(pass_context=True, brief="Finds empty days and links message")
    @commands.check(backend.command_checks.is_authed)
    async def find_blank(self, ctx: commands.Context):
        ...
        for message_id in blank_ids:
            my_message_id, my_channel_id = await fetch_from_tracking_table(
                message_id["message"], self.bot.db
            )
    ...
    async def _old_force_add_message(self, ctx: commands.Context):
        ...
        if len(ctx.message.content.split(" ")) != 3:
            await ctx.send(
                "I need a channel ID then a message ID in the format of "
        ...
        try:
            fetched_message = await fetched_channel.get_message(
                fetched_channel, message
            )
        except discord.NotFound as DNF:
            await ctx.send(str(DNF))
            return
    ...
    async def _old_force_alter_day(self, ctx: commands.Context):
        ...
        if len(ctx.message.content.split(" ")) != 4:
            await ctx.send(
                "I need a channel ID then a message ID then a day (as a int) in the format of "
        ...
        try:
            fetched_message: discord.Message = await fetched_channel.fetch_message(
                int(message)
            )
        except discord.NotFound as DNF:
            await ctx.send(str(DNF))
